...
class RectangularRoom(object):
    ...
    def __init__(self, width, height):
        ...
        #initialize dict of tuples for tiles locations
    ...
    def isPositionInRoom(self, pos):
        ...
        """
        if (math.floor(pos.getX()),math.floor(pos.getY())) in self.tiles.keys():
            return True
        else:
            return False
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        while not self.room.isPositionInRoom(X):
            self.direction = random.random()*360.0
            X = self.position.getNewPosition(self.direction,self.speed)
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        ...
        for robits in range(num_robots):
            robots[robits] = robot_type(room, speed)
        ...
        while coverage < min_coverage:
            for robits in range(num_robots):
                robots[robits].updatePositionAndClean()                
            steps += 1
            coverage = float(room.getNumCleanedTiles())/float(room.getNumTiles()) 
            
        steps_list.append(steps)

    return float(sum(steps_list))/float(len(steps_list))
